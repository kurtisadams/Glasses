function [gen, discrim, objective] = agn( varargin )
    ...
    % progress img name
    opts.prog_img_name = 'progress_agn.png';
    % attack settings and params
    opts.attack = 'dodge';
    opts.targets = 142;
    opts.face_im_paths = {'data/Danh/aligned_vgg_ms1.png'};
...
function [gen, obj_d, obj_f] = gen_update(opts, discrim, gen, ims, z, face_net, face_ims, masks)
    ...
    n = size(z,1);
    [face_ims, targets] = sample_ims(face_ims, n, opts);
    face_net = switch_to_our_layer(face_net, targets); % switch last layer to "our loss"
    masks = masks(:,:,:,1:n);
    face_ims = faces_wearing_gen(opts, face_ims, gen_out, masks);
    res_faces = vl_simplenn(face_net, face_ims, 1, [], ...
                    'backPropDepth', +inf);
	dgdz_f = res_faces(1).dzdx;
    dgdz_f = gen_facenet_gradients(opts, dgdz_f, masks);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [face_ims, move_info] = faces_wearing_gen(opts, face_ims, gen_out, masks)
    % Resize gen_out
    n = size(gen_out,1);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function dgdz_f = gen_facenet_gradients(opts, dgdz_f, masks)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function net = switch_to_our_layer(net, targets)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [ims, targets] = sample_ims(ims, n, opts)
