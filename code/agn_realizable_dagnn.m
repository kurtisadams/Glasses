function [gen, discrim, objective] = agn_realizable_dagnn( varargin )
    ...
    % progress img name
    opts.prog_img_name = 'progress_agn_realizable_dagnn.png';
    % attack settings and params
    opts.attack = 'dodge';
    opts.targets = 142;
    opts.face_im_paths = {'data/demo-data2/aligned_vgg_ms1.png'};
...
function done = objective_met(opts, face_net, face_ims, masks, gen)
    ...
    for i_g = 1:opts.batch_size/2
        ...
        for i_f = 1:opts.batch_size/2:n
            ...
            % align
            tforms = opts.alignment.tforms(i_f:i_f+n_iter-1);
            face_ims_aligned = align(face_ims_iter, tforms, opts.alignment.im_size);
            % classify the faces
            ...
            for i_t = 1:n_iter
                probs(i_f+i_t-1) = gather(res_faces.value(:,:,targets(i_t),i_t));
            end
        end
...
function [gen, obj_d, obj_f] = gen_update(opts, discrim, gen, ims, z, face_net, face_ims, masks)
    ...
    n = size(z,1);
    [face_ims, targets, tforms, eyeglass_tforms] = sample_ims(face_ims, n, opts);
    masks = masks(:,:,:,1:n);
    face_ims = faces_wearing_gen(opts, face_ims, gen_out, masks, tforms, eyeglass_tforms);
    ...
    dgdz_f = gen_facenet_gradients(opts, dgdz_f, masks, tforms, eyeglass_tforms);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function face_ims = faces_wearing_gen(opts, face_ims, gen_out, masks, tforms, eyeglass_tforms)
    % Resize gen_out
    n = size(gen_out,1);
...
% recognition DNN
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function dgdz_f = gen_facenet_gradients(opts, dgdz_f, masks, tforms, eyeglass_tforms)
    % reverse alignment
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% switch last layer to softmax
% (by removing the "our objective/loss"
% layer)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [ims, targets, tforms, eyeglass_tforms] = sample_ims(ims, n, opts)
